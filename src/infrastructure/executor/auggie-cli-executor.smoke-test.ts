...
async function main(): Promise<void> {
  const tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'aeos-auggie-smoke-'));
  ...
  try {
    ...
    const executor = new AuggieCliExecutor({ timeoutMs: 90_000, maxTurns: 8 });
    const invocation: ExecutorInvocation = {
      ...
      outputPath: path.join(tmpDir, 'auggie-summary.md'),
      ticketId: 'SMOKE-AUGGIE-1',
    ...
    if (!result.ok) {
      console.error(`✗ AuggieCliExecutor failed: ${result.reason}`);
      process.exit(1);
    }

    const updated = await fs.promises.readFile(targetFile, 'utf8');
    if (!updated.includes('NEW')) {
      console.error('✗ target.txt was not modified as expected');
      process.exit(1);
    }
    console.log('✓ target.txt was modified by agentic Auggie run');
    ...
    console.log('Auggie smoke test passed.');
  } finally {
...
const timer = setTimeout(() => {
  console.error('✗ Auggie smoke test timed out after 120s');
  process.exit(1);
}, TIMEOUT_MS);
...
main()
  ...
  .catch((err: unknown) => {
    clearTimeout(timer);
    console.error('✗ Auggie smoke test failed with unexpected error:', err);
    process.exit(1);
  });
